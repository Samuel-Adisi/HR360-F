...
// Debug toggle:
// When true, this screen renders minimal UI (no chart components / heavy sections)
// to isolate whether the tab-freeze is caused by Attendance's heavy rendering.
const ATTENDANCE_MINIMAL_MODE = false;
...
export default function AttendanceDashboard() {
  ...
  const [chartKit, setChartKit] = useState(null);

  // react-native-chart-kit SVG rendering can be heavy on iOS.
  // Defer charts until after the initial tab transition settles.
  useEffect(() => {
    if (ATTENDANCE_MINIMAL_MODE) return;
    // #region agent log
    fetch(
      "http://127.0.0.1:7435/ingest/573e11c3-4929-47fe-8a5b-0b9558470170",
      {
        ...
        body: JSON.stringify({
          sessionId: "021120",
          location: "app/(tabs)/attendance/index.jsx:AttendanceDashboardMount",
          message: "Attendance screen mounted",
          hypothesisId: "H4_attendanceScreenMounted",
          data: { routeKey: "attendance/index" },
          timestamp: Date.now(),
        }),
      },
    ).catch(() => {});
    ...
    InteractionManager.runAfterInteractions(() => {
      // Lazily load to avoid iOS jank caused by module initialization.
      try {
        // eslint-disable-next-line global-require
        const kit = require("react-native-chart-kit");
        setChartKit(kit);
      } catch (e) {
        setChartKit(null);
      } finally {
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: "#F5F6FA" }}>
        ...
        >
          ...
          <Text style={{ marginTop: 10, fontSize: 13, color: "#64748B" }}>
            Minimal render mode (no charts / data)
          </Text>
        </View>
      </SafeAreaView>
    );
      ...
      >
        ...
        <View style={styles.card}>
          ...
          {chartsReady && chartKit ? (
            ...
            <View style={styles.chartPlaceholderBars} />
          )}
        </View>
        ...
        <View style={styles.card}>
          ...
          {chartsReady && chartKit ? (
            ...
            <View style={styles.chartPlaceholderLine} />
          )}
        </View>
